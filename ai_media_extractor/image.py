            ...
            if isinstance(data, dict) and data.get("data"):
                ...
                for message in message_snapshot:
                    ...
                    for m2 in message["content_block"]:
                        ...
                        if not isinstance(json_data2, dict) or not json_data2.get("creation_block"):
                            continue
                        creations = json_data2["creation_block"].get("creations", [])

                        for creation in creations:
                            if not isinstance(creation, dict):
                                continue
                            image_data = creation.get("image")
                            image_raw = image_data.get("image_ori_raw") if isinstance(image_data, dict) else None
                            if not isinstance(image_raw, dict) or not image_raw.get("url"):
                                continue
                ...
                for message in message_snapshot:
                    ...
                    for m2 in message["content_block"]:
                        ...
                        if isinstance(json_data2, dict) and json_data2.get("creation_block"):
                            creations = json_data2["creation_block"].get("creations", [])
                            for creation in creations:
                                if not isinstance(creation, dict):
                                    continue
