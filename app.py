...
from ai_media_extractor.image import doubao_image_parse, qianwen_image_parse
from ai_media_extractor.video import doubao_video_parse, yunque_video_parse
from ai_media_extractor.qianwen import qianwen_my_image_parse, qianwen_my_video_parse

app = FastAPI(title="AI 素材提取器 API", description="从支持的平台对话链接中提取图片和视频资源", version="0.1.0")
...
async def parse_doubao(request: DouBaoRequest):
    try:
        ...
        elif "qianwen.my.cn/share/chat/" in str(request.url):
            result = await qianwen_my_image_parse(str(request.url), return_raw=request.return_raw)
        else:
...
async def parse_doubao_get(url: str, return_raw: bool = False):
    try:
        ...
        elif "qianwen.my.cn/share/chat/" in url:
            result = await qianwen_my_image_parse(url, return_raw=return_raw)
        else:
...
async def parse_video(request: VideoRequest):
    try:
        ...
        elif "qianwen.my.cn/share/chat/" in url_str:
            video_data = await qianwen_my_video_parse(url_str, return_raw=request.return_raw)
        else:
...
async def parse_video_get(url: str, return_raw: bool = False):
    try:
        if "doubao.com" in url:
            ...
            video_data = await qianwen_my_video_parse(url, return_raw=return_raw)
        else:
