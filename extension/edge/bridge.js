...
window.addEventListener('ai-media-extractor-fallback-request', event => {
    ...
    chrome.runtime.sendMessage({ type: 'doubao-fetch-fallback', url: detail.url }, response => {
        ...
        window.dispatchEvent(new CustomEvent('ai-media-extractor-fallback-response', {
            detail: { id: detail.id, ok: Boolean(response?.ok) && !error, data: response?.data, error: error || response?.error }
        }));
    });
});
